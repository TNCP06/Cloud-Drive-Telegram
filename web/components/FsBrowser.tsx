...
// Modal penjelajah file laptop. Mengembalikan PATH ASLI (absolut) ke pemanggil.
// mode "dir"  → pilih folder (untuk game)
// mode "file" → pilih satu file (untuk media)
      ...
      <div className="fsb" onClick={(e) => e.stopPropagation()}>
        <div className="fsb-head">
          <div className="fsb-title">
            ...
            {mode === "dir" ? "Pilih folder di laptop" : "Pilih file di laptop"}
          </div>
          <button className="fsb-x" onClick={onClose} aria-label="Tutup">
        ...
        <div className="fsb-bar">
          <button className="fsb-up" disabled={!listing?.parent} onClick={() => listing?.parent && load(listing.parent)}>
            <Icon name="back" size={15} /> Naik
          </button>
          <span className="fsb-cwd" title={listing?.cwd}>{listing?.cwd ?? "…"}</span>
          <button className="fsb-up" onClick={() => load(listing?.cwd)} title="Muat ulang">
        ...
        <div className="fsb-list scroll">
          {loading ? (
            <div className="fsb-state"><span className="spinner" /> Memuat…</div>
          ) : listing?.error ? (
            <div className="fsb-state err">{listing.error}</div>
          ) : listing && listing.entries.length === 0 ? (
            <div className="fsb-state">Folder kosong.</div>
          ) : (
        ...
        <div className="fsb-foot">
          {mode === "dir" && listing?.cwd && (
            <button className="btn primary" onClick={() => pick(listing.cwd)}>
              <Icon name="check" size={15} /> Pilih folder ini
            </button>
          )}
          <button className="btn subtle" onClick={onClose}>Batal</button>
        </div>
      </div>
    </div>
  );
