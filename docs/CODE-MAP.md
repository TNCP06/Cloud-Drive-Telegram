...
- `DriveApp.tsx` — top-level app shell/state (largest component; its pure view-model logic — section
  ...
  **collapsed behind an icon** (`mobileSearchOpen` + `.search.mob-open`) so the breadcrumb keeps the
  ...
  active the breadcrumb is replaced by "Search results" + a match count. The breadcrumb itself collapses
  past 3 levels (first · … · parent · current, the ellipsis opening a menu of the skipped folders) and every
  crumb carries a `title`, so a truncated folder name is still readable on hover. Folder navigation: **Backspace** steps back through visited folders,
...
- `PreviewDrawer.tsx` — item detail + on-demand gallery (`getGallery`) + **video streaming**: `isPartStreamableVideo()` detects if the active media part has a browser-playable extension (.mp4/.webm/.m4v/.mov) and renders **`VideoPlayer.tsx`** (a **Plyr** player) sourced from `/api/stream/{partId}`. The drawer keeps only `Esc` (close) and `Shift+←/→` (jump between parts/files) — Plyr owns the media shortcuts (←/→ seek 5s via `seekTime`, `f` fullscreen, `m` mute, space play); the document keydown is capture-phase so `Shift+arrows` are intercepted before Plyr's global handler. All action buttons are removed from this drawer's top bar (delegated entirely to the external card/row kebab menus). Supports `detailsOnly` mode to render the metadata/edit panel as a standalone popup without the full-screen photo/video stage layer. **Stills:** renders progressive image loading (instant base64 thumbnail blur/placeholder with smooth fade-in to high-res `/api/stream/{partId}`, backed by browser `Cache-Control` and automatic background prefetching of adjacent ±1 photos/files for zero-lag navigation). The rotate button / `R` turns the photo a quarter at a time, and a quarter turn adds
  `.is-quarter-turn`, which swaps the image box to `100cqh`/`100cqw` (the stage is a size container) so the
  turned photo refits the stage instead of keeping its pre-turn footprint. Touch adds pinch-to-zoom (1–5×),
  double-tap 1× ↔ 2.5×, and drag-to-pan (`zoom` state → one `translate/scale/rotate` transform, pan clamped to
  the media's overflow); swipe only pages while at 1×, and zoom/rotation reset on any part or file change.
  **Touch behaviour:** a tap on the stage that moved under 10px toggles `chromeHidden` (the gallery-app immersive view — top bar, nav, pill and, on touch only, the filmstrip clear off the media); anything past 40px horizontally stays a swipe between parts/files. Video is excluded — Plyr drives the same show/hide there and the CSS mirrors our chrome onto its `.plyr--hide-controls`. Entering fullscreen calls `screen.orientation.lock()` to match the media's aspect (best-effort: fullscreen-only API, absent on iOS, refused under an OS rotation lock) and, on a viewport ≤800px, folds the filmstrip away — restoring the previous collapse state on exit. For non-media single-part files whose `fileTypeFor().preview` is `pdf|text|word|sheet`, the stage renders **`DocPreview.tsx`** instead of the static cover/icon (PDF adds `.has-pdf-stage` with a 52px top offset so native browser PDF print/zoom controls are unobscured). On the video stage the top bar has an **"Add subtitle"** button (CC icon) opening **`SubtitleDialog.tsx`** — three sources: upload a local SRT/VTT/ASS file (`/api/subtitles/{id}/manual`), attach a subtitle file already stored on the drive (`listDriveSubtitleFiles` server action in `actions/subtitles.ts` + `/api/subtitles/{id}/from-part`), or extract the video's embedded softsub streams (`/api/subtitles/{id}/extract`, polled every 3 s). The language comes from a dropdown but a `Movie.id.srt`-style file name overrides it (`guessLang`); on success the dialog bumps `subsBump` (part of the `VideoPlayer` key) to remount the player so the new track shows in the CC menu immediately. `FsBrowser.tsx` — laptop folder picker (drives `listDir`).
- `DocPreview.tsx` — inline document preview on the viewer stage. **PDF** → native `<iframe>`
  ...
  → fetched as text into a `<pre>` (chunked to first 1,000 lines with an expand button for fast rendering of giant text/code files). **Word (.docx)** → `mammoth` (dynamically imported browser build
...
- `VideoPlayer.tsx` — **Plyr** wrapper for the lightbox stage. Fills the whole `.viewer-stage` from the first frame (Plyr's wrapper/video are 100%×100%; the frame is letterboxed via `object-fit: contain`) so it never starts tiny while the stream loads. Plyr's `clickToPlay` is disabled and clicks are split by geometry: on the displayed (contain-fitted) frame → play/pause; on the letterbox → `onRequestClose` (skipped while fullscreen); on controls → Plyr. Poster dims (the data-URL thumbnail) seed the letterbox hit-test before the video reports its own size. Volume/mute are persisted to `localStorage` (`video-volume`/`video-muted`) and restored on `ready` — Plyr's own `storage` is off — so they never reset when switching videos. It initializes with `previewThumbnails: { src: '/api/seek-preview/{partId}?wait=true' }` which blocks on the streamer up to 60s so hover thumbnails appear dynamically mid-watch once background generation finishes. Includes `"fullscreen"` in Plyr controls and delegates `player.fullscreen.toggle` to `onToggleFullscreen` so the entire `.viewer` container enters fullscreen (keeping top bar, filmstrip, and Plyr controls consistent). Control-bar tooltips come from one `i18n` map in the shape `<action> (<key>)`, and **fullscreen reads the same
in both directions** — its toggle is delegated to the viewer, so Plyr never learns it is fullscreen and its
default "Enter fullscreen" label would keep claiming that while already there. Plyr ships no loop control, so
`addLoopButton()` injects one (`.tcd-loop`, a `.plyr__tooltip` span like Plyr's own, `plyr__control--pressed`
+ accent colour while looping) at the head of the right-hand cluster; the viewer's `P` (loop) and `I` (PIP)
shortcuts just click the buttons, so the lit button stays the single source of truth. The geometry click-split
is **desktop-only**: on a coarse pointer the handler bows out so a tap just reveals Plyr's controls (Plyr's own touch behaviour — tap shows, 3 s of stillness hides), and play/pause happens on the control buttons, as on any mobile player.
