...
- **Thumbnails are per-part** (`thumbnails.part_id`). An item's cover = thumbnail of the part
  ...
  on demand via `getGallery()`. `thumbnails.source` records who wrote the image and therefore who
  may replace it: `'telegram'` (the built-in thumbnail — only ~320 px for a video, so treated as a
  placeholder), `'ffmpeg'` (a frame the streamer pulled from the video itself, `stream_poster.py`),
  `'manual'` (uploaded from the dashboard — never overwritten by either).
