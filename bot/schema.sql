...
-- Files parked on the VPS instead of Telegram — only PikPak/OpenList downloads the owner sent to
--   dest='vps'. rel_path is relative to /staging/_unpack/_keep. The web lists them (download /
--   upload-to-Telegram / delete-now); the unpack worker's idle sweep deletes file+row once
--   expires_at passes (UNPACK_KEEP_TTL_H, 72h). Unpack outputs never land here: an oversized video
--   is cut into playable segments by the watcher and anything else is raw-split.
CREATE TABLE IF NOT EXISTS unpack_kept (
    id         BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    item_id    BIGINT,                              -- source archive item (no FK: a purged archive must not delete the kept file row)
    file_name  TEXT NOT NULL,
    rel_path   TEXT NOT NULL,
    size       BIGINT NOT NULL DEFAULT 0,
    expires_at TEXT NOT NULL,                       -- now_text() format; sweep deletes past this
    created_at TEXT NOT NULL DEFAULT now_text()
);

DROP TABLE IF EXISTS kept_compress_jobs;   -- manual re-encode queue, replaced by watcher segmentation
