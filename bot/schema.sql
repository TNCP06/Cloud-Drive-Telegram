...
-- Extracted files the unpack worker KEPT on the VPS instead of re-uploading (> 2 GB Telegram cap).
--   rel_path is relative to /staging/_unpack/_keep. The web lists them (download + delete-now);
--   the unpack worker's idle sweep deletes file+row once expires_at passes (UNPACK_KEEP_TTL_H, 72h).
...
-- Manual compress queue for kept files (web inserts crf; the unpack worker runs ffmpeg on the
-- VPS copy and replaces it in place when the result is smaller).
CREATE TABLE IF NOT EXISTS kept_compress_jobs (
    ...
    status     TEXT NOT NULL DEFAULT 'queued'
                 CHECK (status IN ('queued','running','done','failed')),
    message    TEXT,                               -- live progress ("compressing… 40%") / result
    created_at TEXT NOT NULL DEFAULT now_text(),
    updated_at TEXT NOT NULL DEFAULT now_text()
);
