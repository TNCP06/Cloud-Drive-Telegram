...
# Files parked on the VPS instead of Telegram: PikPak downloads the owner explicitly sent to
# dest='vps'. They live under _keep/ with an unpack_kept row + expiry; the web dashboard lists
# them (download / upload-to-Telegram / delete-now) and the idle sweep removes them once expired.
# Unpack outputs never land here — oversized ones are segmented/split by the watcher instead.
...
async def _stage_outputs(db, outdir, unpacked_folder, jid, item_id):
    """Move each extracted file into its own staging dir and queue an upload_jobs row.

    Everything goes to Telegram, whatever its size: the watcher cuts a video over the 2 GB cap
    into playable segments and raw-splits anything else, so nothing has to squat on the VPS.
...
async def _process(client, channel, db, job):
    ...
    try:
        ...
        if cached:
            ...
            # stored size. Outputs are MOVED (renamed) into upload staging, never copied, so this
            # holds regardless of their size. A > 2 GB video additionally needs room for its
            # ffmpeg segments — the watcher checks that itself right before splitting.
            DISK_FACTOR = 2.3
            try:
        ...
        n = await _stage_outputs(db, outdir, title, jid, item_id)
        if n == 0:
